...
const prices = regions.flatMap((region, regionIndex) =>
    cropCatalog.map((item, cropIndex) => {
        const base = item.base * (0.88 + (regionIndex % 8) * 0.035);
        const price = Math.round(base / 1000) * 1000;
        const changeValue = ((regionIndex * 3 + cropIndex * 2) % 17 - 5) / 10;
        return {
            crop: item.crop,
            region,
            price: price.toLocaleString("en-TZ"),
...
function renderPrices(region = "all") {
    const filtered = prices.filter(item => region === "all" || item.region === region);
    grid.innerHTML = filtered.map(item => `
        <article class="price-card">
            ...
            <div class="price">TZS ${item.price} <small>/ 100 kg</small></div>
...
renderPrices();
regionFilter.addEventListener("change", event => renderPrices(event.target.value));
